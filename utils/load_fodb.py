...
def load_fodb(classes, path, dataset):  
    examples = []
    labels = []
    ...
    for DEVICE in os.listdir(f'{path}/{dataset}'):
        if '.DS_Store' not in DEVICE:
            classes = f'{path}/{dataset}/{DEVICE}'
            for CLASS in os.listdir(classes):
                    images = f'{path}/{dataset}/{DEVICE}/{CLASS}'
                    for IMAGE in os.listdir(images):
                        if '.DS_Store' not in IMAGE:
                            example = f'{path}/{dataset}/{DEVICE}/{CLASS}/{IMAGE}'
